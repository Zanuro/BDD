...
DECLARE
  ...
  zone_coordinates VARCHAR(45);
  zone_stock INT;
  total_stock INT;
BEGIN
  -- Get zone id
  SELECT nombre_zona, coordenadas INTO zone_name, zone_coordinates
  FROM empleado_trabaja_zona
  WHERE dni=NEW.dni_empleado;

  IF zone_name IS NULL THEN
    -- Cancel insert
    RAISE EXCEPTION 'El empleado no tiene zona asignada.';
  END IF;

  -- Get zone stock
  SELECT cantidad INTO zone_stock
  FROM producto_zona
  WHERE codigo=NEW.codigo AND nombre_zona=zone_name AND coordenadas=zone_coordinates;

  IF zone_stock IS NULL THEN
    -- Cancel insert
    RAISE EXCEPTION 'No existe el producto en la zona o su cantidad es nula.';
  END IF;

  IF zone_stock < NEW.cantidad THEN
    -- Cancel insert
    RAISE EXCEPTION 'La cantidad de producto en la zona es inferior a la solicitada.';
  END IF;

  -- Update zone stock in the table producto_zona
  zone_stock := zone_stock - NEW.cantidad;
  
  UPDATE producto_zona
  SET cantidad=zone_stock
  WHERE codigo=NEW.codigo AND nombre_zona=zone_name AND coordenadas=zone_coordinates;
...
BEFORE INSERT ON cliente_compra_producto
FOR EACH ROW
EXECUTE PROCEDURE modify_stock();


CREATE OR REPLACE FUNCTION update_product_stock()
RETURNS trigger AS $update_product_stock$
BEGIN
  -- Update product stock
  UPDATE producto
  SET stock=(SELECT SUM(cantidad)
            FROM producto_zona
  ...
  RETURN NEW;
END;
$update_product_stock$ LANGUAGE plpgsql;


CREATE TRIGGER update_stock
AFTER INSERT OR UPDATE ON producto_zona
FOR EACH ROW
...
INSERT INTO cliente_compra_producto (dni_cliente, codigo, cantidad, fecha, dni_empleado) VALUES ('56565656B', '9999', 1, '2019-04-11 10:05:10', '12121212C');
INSERT INTO cliente_compra_producto (dni_cliente, codigo, cantidad, fecha, dni_empleado) VALUES ('45454545B', '1010', 3, '2019-04-11 10:00:00', '21212121D');
