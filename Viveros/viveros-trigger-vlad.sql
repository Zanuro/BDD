...
BEGIN
 ...
 SELECT stock INTO total_stock FROM producto WHERE new.codigo = codigo;
  ...
  IF new.cantidad <= total_stock THEN
	...
	SELECT coordenadas INTO coord FROM empleado_trabaja_zona INNER JOIN empleado ON empleado_trabaja_zona.dni = empleado.dni INNER JOIN cliente_compra_producto ON empleado.dni = new.dni_empleado;	

        SELECT cantidad INTO zone_stock FROM producto_zona INNER JOIN producto ON producto.codigo = producto_zona.codigo WHERE producto_zona.coordenadas = coord AND producto_zona.codigo=new.codigo;

	zone_stock := zone_stock-new.cantidad;

	IF zone_stock >= 0 THEN

        	UPDATE producto_zona SET cantidad=zone_stock WHERE producto_zona.codigo = new.codigo AND producto_zona.coordenadas = coord;
	ELSE
