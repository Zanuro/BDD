...
RETURNS trigger AS $modify_stock$
DECLARE
  total_stock INT;
  number_of_zones INT;
  random_zone INT;
...
BEGIN
 ...
 SELECT stock INTO total_stock FROM producto WHERE new.codigo = codigo;
 
  IF new.cantidad <= total_stock THEN

 	total_stock := total_stock - new.cantidad;
 	UPDATE producto SET stock=total_stock WHERE new.codigo = codigo;
 	--Comprobar que resta de una sola zona
	
	SELECT nombre_zona, coordenadas INTO zone, coord FROM empleado_trabaja_zona INNER JOIN empleado ON empleado_trabaja_zona.dni = empleado.dni INNER JOIN cliente_compra_producto ON empleado.dni = new.dni_empleado;	
	
	RAISE NOTICE 'Nombre zona: % - Coordenadas: %',  zone, coord;

        SELECT cantidad INTO zone_stock FROM producto_zona INNER JOIN producto ON producto.codigo = producto_zona.codigo WHERE producto_zona.coordenadas = coord AND producto_zona.nombre_zona = zone AND producto_zona.codigo=new.codigo;
	
	RAISE NOTICE 'Stock zona: %',  zone_stock;
	zone_stock := zone_stock-new.cantidad;
	RAISE NOTICE 'Stock despues de restar compra: %',  zone_stock;

	IF zone_stock >= 0 THEN

        	UPDATE producto_zona SET cantidad=zone_stock WHERE producto_zona.codigo = new.codigo AND producto_zona.coordenadas = coord AND producto_zona.nombre_zona = zone;
	ELSE
	        RAISE EXCEPTION 'La insercion no se ha podido realizar ya que el empleado no trabaja en una zona que tenga esa cantidad de producto'; 

	END IF;

 	RETURN NEW;
  ELSE
	RAISE EXCEPTION 'No hay tanta cantidad en stock';
...
END;
$modify_stock$ LANGUAGE plpgsql;


CREATE TRIGGER mod_stock
AFTER INSERT ON cliente_compra_producto
FOR EACH ROW
EXECUTE PROCEDURE modify_stock();
...
INSERT INTO producto (codigo, descripcion, precio, stock) VALUES ('1234', 'Helecho', 7.15, 16);
INSERT INTO producto (codigo, descripcion, precio, stock) VALUES ('2323', 'Orquidea', 10.22, 10);
INSERT INTO producto (codigo, descripcion, precio, stock) VALUES ('9999', 'Tierra 10 kg.', 2.30, 8);
INSERT INTO producto (codigo, descripcion, precio, stock) VALUES ('1010', 'Abono 10 kg.', 9.21, 14);
...
INSERT INTO producto_zona (codigo, nombre_zona, coordenadas, cantidad) VALUES ('9999', 'Almacen', '28°28''00"N 16°15''00"O', 2);
INSERT INTO producto_zona (codigo, nombre_zona, coordenadas, cantidad) VALUES ('1010', 'Almacen', '28°28''00"N 16°15''00"O', 10);
INSERT INTO producto_zona (codigo, nombre_zona, coordenadas, cantidad) VALUES ('1234', 'Entrada', '28°29''N 16°19''O', 6);
INSERT INTO producto_zona (codigo, nombre_zona, coordenadas, cantidad) VALUES ('2323', 'Entrada', '28°29''N 16°19''O', 5);
INSERT INTO producto_zona (codigo, nombre_zona, coordenadas, cantidad) VALUES ('9999', 'Almacen', '28°29''N 16°19''O', 6);
INSERT INTO producto_zona (codigo, nombre_zona, coordenadas, cantidad) VALUES ('1010', 'Almacen', '28°29''N 16°19''O', 4);
...
INSERT INTO cliente_compra_producto (dni_cliente, codigo, cantidad, fecha, dni_empleado) VALUES ('56565656B', '1234', 5, '2019-04-11 10:00:00', '12345678A');
INSERT INTO cliente_compra_producto (dni_cliente, codigo, cantidad, fecha, dni_empleado) VALUES ('56565656B', '2323', 1, '2019-04-11 10:05:10', '12345678A');
INSERT INTO cliente_compra_producto (dni_cliente, codigo, cantidad, fecha, dni_empleado) VALUES ('45454545B', '2323', 3, '2019-04-11 10:00:00', '87654321B');
